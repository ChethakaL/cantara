...
export function buildLoiReviewReportHtml(report: LoiReport): string {
  const sections = parseMarkdownSections(report.markdown)

  return generateReportHtml({
    title: 'LOI Review & Comparison',
    subtitle: 'Letter of Intent Comparative Analysis',
    clientName: report.clientName,
    generatedAt: report.generatedAt,
    summaryHtml: sections.length > 0 ? markdownToHtml(sections[0].content) : undefined,
    kpis: [
      { label: 'Workstream', value: 'M&A' },
      { label: 'Report Type', value: 'LOI Review' },
