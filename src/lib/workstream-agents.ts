...
export const SYSTEM_WORKSTREAM_AGENTS: Record<Exclude<Workstream, null>, WorkstreamAgentSelection[]> = {
  ws1: [
    ...
    { agentId: 'legal_entity_search', agentName: 'Legal Reports & Entity Search Agent', documentIds: ['articles_org', 'org_document_amendments', 'annual_reports', 'good_standing_certificate', 'shareholder_agreement', 'ownership_structure', 'business_licenses'] },
    ...
    { agentId: 'tax_liability_review', agentName: 'Tax Liability Review Agent', documentIds: ['tax_returns_3yr', 'irs_941_940_3yr', 'state_unemployment_filings', 'contractor_1099_agreements', 'sales_use_tax_3yr', 'irs_tax_notices_3yr'] },
...
export function getClientWorkstreamAgents(client: {
  ...
  // client's saved workstreamAgents list omits it). For ws1/ws2 alone, only
  // when explicitly added (admin opt-in via "Add Professional Advisors").
  const alwaysIncludeProfessionalAdvisors =
    client.workstream === 'ma' || client.workstream === 'both'
  if (alwaysIncludeProfessionalAdvisors) {
    if (!agents.some(agent => agent.agentId === 'professional_advisors')) {
      agents = [
        ...agents,
        { agentId: 'professional_advisors', agentName: 'Professional Advisors Agent', documentIds: [] },
      ]
