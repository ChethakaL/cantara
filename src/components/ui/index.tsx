...
export function Textarea({ label, className, ...props }: TextareaProps) {
  return (
    <div className="space-y-1.5">
      ...
      <textarea
        className={cn(
          'w-full px-3 py-2.5 text-sm text-slate-800 rounded-lg border border-slate-200 outline-none resize-y transition-all leading-relaxed',
          'focus:border-cantara-gold focus:ring-2 focus:ring-cantara-gold/20',
          'min-h-[120px]',
          className
        )}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={cn('relative bg-cantara-white rounded-2xl shadow-2xl w-full max-h-[90vh] overflow-y-auto', sizeClassName ?? 'max-w-lg')}>
        <div className="flex items-center justify-between p-6 border-b border-slate-100">
          ...
          {!hideCloseButton && <button onClick={onClose} className="text-slate-400 hover:text-slate-600 text-xl leading-none">×</button>}
        </div>
