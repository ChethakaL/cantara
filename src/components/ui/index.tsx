...
export function Button({ variant = 'primary', size = 'md', className, children, ...props }: ButtonProps) {
  ...
  const style = variant === 'primary' ? { background: 'linear-gradient(135deg, #b8922a, #d4a843)' } : {}
  return (
    <button
...
export function Modal({ open, onClose, title, children, sizeClassName }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={cn('relative bg-white rounded-2xl shadow-2xl w-full max-h-[90vh] overflow-y-auto', sizeClassName ?? 'max-w-lg')}>
...
export function ProgressBar({ value, className }: { value: number; className?: string }) {
  return (
    <div className={cn('w-full h-1.5 bg-slate-100 rounded-full overflow-hidden', className)}>
      <div className="h-full rounded-full transition-all" style={{ width: `${value}%`, background: 'linear-gradient(90deg, #b8922a, #d4a843)' }} />
    </div>
  )
}
