'use client'
// WS1-9 — Upload Screen for Business Permits & Zoning Documents

import { useEffect, useRef, useState } from 'react'
import { Save } from 'lucide-react'
import type { UploadedDoc } from '@/hooks/useWS19Analysis'

// All document slots for permits & zoning analysis
...
export default function WS19Uploader({ clientId, onDocumentsReady, onAnalyze, isLoading }: Props) {
  ...
  useEffect(() => {
    ...
    async function loadDraft() {
      try {
        const res = await fetch(`/api/client-data/${clientId}?section=permitsZoningDraft`)
        if (!res.ok) return
