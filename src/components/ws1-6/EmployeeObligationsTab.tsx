...
interface EmployeeObligationsTabProps {
  clientId: string
  clientName: string
  state?: string
  dba?: string
      ...
      <div className="-m-6 bg-stone-50 min-h-[500px] p-6 lg:p-8">
        <div className="max-w-4xl mx-auto">
          <Card className="p-10 border-stone-200 shadow-sm">
            <WS16Uploader onDocumentsReady={setDocuments} onAnalyze={analyze} isLoading={isRunning} />
    ...
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden border-stone-200 shadow-sm bg-white ring-1 ring-stone-950/5">
        ...
        <div className="min-h-[500px]">
          ...
          {activeTab === 'agreements' && <AgreementsTab report={report} flags={flags} onConfirm={id => handleFlagUpdate(id, 'confirmed')} onNA={id => handleFlagUpdate(id, 'na')} />}
          {activeTab === 'noncompetes' && <NonCompetesTab report={report} flags={flags} onConfirm={id => handleFlagUpdate(id, 'confirmed')} onNA={id => handleFlagUpdate(id, 'na')} />}
          {activeTab === 'benefits' && <BenefitsTab report={report} flags={flags} onConfirm={id => handleFlagUpdate(id, 'confirmed')} onNA={id => handleFlagUpdate(id, 'na')} />}
          {activeTab === 'contractors' && <ContractorsTab report={report} flags={flags} onConfirm={id => handleFlagUpdate(id, 'confirmed')} onNA={id => handleFlagUpdate(id, 'na')} />}
          {activeTab === 'keypeople' && <KeyPeopleTab report={report} flags={flags} onConfirm={id => handleFlagUpdate(id, 'confirmed')} onNA={id => handleFlagUpdate(id, 'na')} />}
          {activeTab === 'review' && <CraigReviewTab flags={flags} onConfirm={id => handleFlagUpdate(id, 'confirmed')} onNA={id => handleFlagUpdate(id, 'na')} report={report} onRelease={handleRelease} isReleasing={releasing} />}
        </div>
      </Card>
