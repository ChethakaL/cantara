...
import { WS110_SYSTEM_PROMPT, buildWS110ContextBlock } from '@/lib/ws1-10/prompt'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const contextBlock = buildWS110ContextBlock({
      clientName,
      state: state ?? 'Unknown',
      dba,
      entityType,
      ...
      documents.map(async (doc: any) => {
        ...
        if (
          ...
          try {
            ...
            console.error(`[WS1-10] Image resize failed for ${name}:`, err)
            return {
              type: 'image' as const,
              source: {
                type: 'base64' as const,
        ...
        if (ext.endsWith('.xlsx')) {
          try {
            ...
            console.error(`[WS1-10] XLSX parsing failed for ${name}:`, err)
          }
        }
        ...
        if (ext.endsWith('.docx') || doc.mediaType === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
          try {
            ...
            console.error(`[WS1-10] DOCX parsing failed for ${name}:`, err)
          }
        }
    ...
    const readableStream = new ReadableStream({
      async start(controller) {
        ...
        for (let attempt = 1; attempt <= MAX_UPSTREAM_ATTEMPTS; attempt++) {
          ...
          try {
            ...
            console.log('[WS1-10] Claude response complete. Total length:', fullResponse.length)
            controller.close()
            return
          } catch (error) {
            ...
            console.error(`[WS1-10] Attempt ${attempt} failed:`, error)
      ...
      cancel() {
        console.log('[WS1-10] Stream cancelled by client.')
        activeStream?.controller.abort()
      },
    })
    ...
    console.error('[WS1-10 Analyze Error]:', error)
...
function formatError(error: unknown) {
  const message = error instanceof Error ? error.message : 'Legal entity search analysis failed.'
  if (isRetryableError(error)) {
    return `Transient upstream connection error while running WS1-10 Legal Entity Search analysis. ${message}`
  }
  return message
}
