...
import { WS19_SYSTEM_PROMPT, buildWS19ContextBlock } from '@/lib/ws1-9/prompt'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const contextBlock = buildWS19ContextBlock({
      clientName,
      state: state ?? 'Unknown',
      dba,
      propertyAddress,
      ...
      documents.map(async (doc: any) => {
        ...
        if (
          ...
          try {
            ...
            console.error(`[WS1-9] Image resize failed for ${name}:`, err)
            return {
              type: 'image' as const,
              source: {
                type: 'base64' as const,
        ...
        if (ext.endsWith('.xlsx')) {
          try {
            ...
            console.error(`[WS1-9] XLSX parsing failed for ${name}:`, err)
          }
        }
        ...
        if (ext.endsWith('.docx') || doc.mediaType === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
          try {
            ...
            console.error(`[WS1-9] DOCX parsing failed for ${name}:`, err)
          }
        }
    ...
    const readableStream = new ReadableStream({
      async start(controller) {
        ...
        for (let attempt = 1; attempt <= MAX_UPSTREAM_ATTEMPTS; attempt++) {
          ...
          try {
            ...
            console.log('[WS1-9] Claude response complete. Total length:', fullResponse.length)
            controller.close()
            return
          } catch (error) {
            ...
            console.error(`[WS1-9] Attempt ${attempt} failed:`, error)
      ...
      cancel() {
        console.log('[WS1-9] Stream cancelled by client.')
        activeStream?.controller.abort()
      },
    })
    ...
    console.error('[WS1-9 Analyze Error]:', error)
...
function formatError(error: unknown) {
  const message = error instanceof Error ? error.message : 'Permits & zoning analysis failed.'
  if (isRetryableError(error)) {
    return `Transient upstream connection error while running WS1-9 Permits & Zoning analysis. ${message}`
  }
  return message
}
