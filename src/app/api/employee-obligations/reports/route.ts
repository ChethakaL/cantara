...
export async function GET(req: NextRequest) {
  try {
    ...
    const reports = await (prisma as any).employeeObligationsReport.findMany({
      where: { clientId },
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const target = reportId
      ? await (prisma as any).employeeObligationsReport.findFirst({ where: { id: reportId, clientId } })
      : await (prisma as any).employeeObligationsReport.findFirst({
          where: { clientId },
          orderBy: { createdAt: 'desc' },
        })
