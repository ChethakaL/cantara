...
// Architecture: WS1-6_Employee_Obligations_Architecture.md — Section: Agent Input Specification
...
export async function POST(req: NextRequest) {
  try {
    const { documents, clientName, state, dba, totalEmployeesSelfReported, employmentTypeBreakdown } =
      await req.json()
    ...
    // Architecture spec: context block injected at top of user message
    const contextBlock = buildWS16ContextBlock({
      clientName,
      state: state ?? 'Unknown',
      dba,
    ...
    // Architecture spec — Files Passed to Agent:
    // - PDFs → base64-encoded, passed as a `document` content block
    // - DOCX → converted to plain text via portal pre-processing
    // - XLSX payroll registers → pre-processed into a structured Markdown table
    // - PNG (org chart) → passed as image block for visual analysis
    const contentBlocks = await Promise.all(
      documents.map(async (doc: any) => {
        const name: string = doc.name ?? ''
        const ext = name.toLowerCase()

        // PNG / JPEG org charts → image block
        if (
          ...
          try {
            ...
            // Resize to max 1568px (Claude's sweet spot) if it exceeds Anthropic's 2000px multi-image limit
            ...
            console.error(`[WS1-6] Image resize failed for ${name}:`, err)
        ...
        // PDFs → document block (spec: base64-encoded `document` content block)
        ...
        // Convert XLSX to text (CSV format for LLM reading) as defined in architecture doc
        if (ext.endsWith('.xlsx')) {
          try {
            ...
            console.error(`[WS1-6] XLSX parsing failed for ${name}:`, err)
            // Fallthrough to generic text
          }
        }
        ...
        if (ext.endsWith('.docx') || doc.mediaType === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
          try {
            ...
            console.error(`[WS1-6] DOCX parsing failed for ${name}:`, err)
          }
        }

        // DOCX / plain text (spec: "converted to plain text via portal pre-processing")
        return {
          type: 'text' as const,
          text: `=== DOCUMENT: ${name} ===\n\n${
    ...
    const userContent: any[] = [
      ...
      {
        type: 'text',
        text: `Please analyze the ${documents.length} employment document(s) above for ${clientName}. Produce the full Employee Obligations Report as specified in your instructions. Document names: ${documents.map((d: any) => d.name).join(', ')}`,
      },
    ]
    ...
    const readableStream = new ReadableStream({
        ...
        for (let attempt = 1; attempt <= MAX_UPSTREAM_ATTEMPTS; attempt++) {
          ...
          try {
            activeStream = await client.messages.stream({
              model: resolveModel('claude-sonnet-4-20250514'), // Architecture spec: Model field
              max_tokens: 8000,
              temperature: 0,                    // Architecture spec: Temperature field
              system: WS16_SYSTEM_PROMPT,
            ...
            console.log('[WS1-6] Claude response complete. Total length:', fullResponse.length)
            controller.close()
            return
          } catch (error) {
            ...
            console.error(`[WS1-6] Attempt ${attempt} failed:`, error)
      ...
      cancel() {
        console.log('[WS1-6] Stream cancelled by client.')
        activeStream?.controller.abort()
      },
    })
    ...
    console.error('[WS1-6 Analyze Error]:', error)
...
function formatError(error: unknown) {
  const message = error instanceof Error ? error.message : 'Employee obligations analysis failed.'
  if (isRetryableError(error)) {
    return `Transient upstream connection error while running WS1-6 Employee Obligations analysis. ${message}`
  }
  return message
}
