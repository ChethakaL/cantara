...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body.action === 'approve-email') return NextResponse.json(await approveSalesLeadEmail(id, actor))
    if (body.action === 'reject-email') return NextResponse.json(await rejectSalesLeadEmail(id, actor))
    if (body.action === 'record-call') {
      return NextResponse.json(await recordSalesLeadCall({
        id,
    ...
    if (body.action === 'change-stage') {
      return NextResponse.json(await setSalesLeadStage({
        id,
