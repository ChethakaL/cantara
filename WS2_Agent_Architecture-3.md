...
**The Anthropic API cannot receive Excel files directly.** Excel files must be converted to CSV text client-side using SheetJS before being sent to the API.

#### File Structure — What the Seller Submits

The P&L and Balance Sheet files each contain **3 sheets** — one per fiscal year (e.g., "FY 2022", "FY 2023", "FY 2024"). Each sheet has:
- Columns: GL Code | Account Name | Jan | Feb | Mar | Apr | May | Jun | Jul | Aug | Sep | Oct | Nov | Dec | FY XXXX Total
- The final column ("FY XXXX Total") is an annual sum. **Ignore this column** — do not include it in any monthly calculations. The agent computes its own totals from the 12 monthly columns only.

The conversion step must concatenate all 3 sheets into a single text block before sending to the API, with a clear separator between sheets so the agent knows where one year ends and the next begins.

#### File Routing — Which File Goes to Which Agent

| File | Agent | Notes |
|---|---|---|
| File 1 — Monthly P&L (3 sheets) | WS2-1 only | |
| File 2 — Monthly Balance Sheet (3 sheets) | WS2-1 only | |
| File 3 — Accountant Statements | WS2-1 only | |
| File 4 — AR Aging Detail | WS2-1 only | |
| File 5 — Add-Back Disclosure (items 2.1–2.5 + 3.2) | **WS2-2 only** | Portal holds this file after upload and passes it to WS2-2 — it is NOT sent to WS2-1 |
| QB API data (if connected) | WS2-1 only | Cross-reference source |

**Critical for the engineer:** File 5 must not be passed to the WS2-1 API call. It is uploaded by the seller at the same time as files 1–4, but the portal holds it in storage and only passes it to the WS2-2 API call after Craig clears the WS2-1 HITL gate.

#### SheetJS Conversion Code

```javascript
import * as XLSX from 'xlsx';

/**
 * Converts a multi-sheet Excel workbook to a single concatenated CSV text block.
 * Used for P&L (3 sheets) and Balance Sheet (3 sheets).
 * The annual total column (last column) is stripped before sending to the API.
 */
const convertMultiSheetExcelToText = (file, fileLabel) => {
    ...
    reader.onload = (e) => {
      ...
      const sheetTexts = workbook.SheetNames.map(name => {
        ...
        const stripped = rows.map(row => {
          ...
          if (row.length >= 15) return row.slice(0, 14);
          return row;
        });

        // Convert back to CSV
        const csvText = stripped.map(row =>
          row.map(cell => {
            const s = String(cell ?? '');
      ...
      // Concatenate all sheets with clear separators
      const fullText = `=== INPUT FILE: ${fileLabel} ===\n` +
 ...
 */
const convertSingleSheetExcelToText = (file, fileLabel) => {
  return new Promise((resolve) => {
    const reader = new FileReader();
    reader.onload = (e) => {
      const workbook = XLSX.read(e.target.result, { type: 'binary' });
      ...
      resolve(`=== INPUT FILE: ${fileLabel} ===\n${csvText}`);
    };
    reader.readAsBinaryString(file);
  });
};

// Build the API message content array for WS2-1
const buildWS21MessageContent = async (plFile, bsFile, acctFile, arFile) => {
  const [plText, bsText, acctText, arText] = await Promise.all([
    convertMultiSheetExcelToText(plFile, 'Monthly P&L — 3 Fiscal Years (FY2022, FY2023, FY2024)'),
    convertMultiSheetExcelToText(bsFile, 'Monthly Balance Sheet — 3 Fiscal Years (FY2022, FY2023, FY2024)'),
    convertSingleSheetExcelToText(acctFile, 'Accountant-Prepared Financial Statements — 3 Fiscal Years'),
    convertSingleSheetExcelToText(arFile, 'Accounts Receivable Aging Detail'),
  ]);

  return [
    { type: "text", text: plText },
    { type: "text", text: bsText },
    { type: "text", text: acctText },
    { type: "text", text: arText },
    { type: "text", text: "Please analyze the above financial data and produce the TTM Financial Analysis Report as specified in your instructions." }
  ];
};

// Build the API message content array for WS2-2
// Called AFTER Craig clears the WS2-1 HITL gate.
// Receives WS2-1 structured output + the add-back disclosure file (File 5).
const buildWS22MessageContent = async (ws21Output, addBackFile) => {
  const addBackText = await convertSingleSheetExcelToText(
    addBackFile,
    'Seller Add-Back Disclosure (Items 2.1–2.5 and 3.2)'
  );

  return [
    {
      type: "text",
      text: `=== WS2-1 STRUCTURED OUTPUT (36-Month Financial Model) ===\n${JSON.stringify(ws21Output, null, 2)}`
    },
    { type: "text", text: addBackText },
    { type: "text", text: "Please perform the EBITDA recast as specified in your instructions using the financial model and add-back disclosure above." }
  ];
};
```
...
You will receive 4 financial files as CSV text blocks, labeled with === INPUT FILE === headers. Before doing anything else, read the file labels and understand the structure you are receiving.

UNDERSTANDING THE INPUT STRUCTURE:

The Monthly P&L file and the Monthly Balance Sheet file each contain DATA FROM 3 SEPARATE SHEETS — one per fiscal year. The sheets are concatenated into a single text block with separators:
  === SHEET: FY 2022 ===   (months Jan-2022 through Dec-2022)
  === SHEET: FY 2023 ===   (months Jan-2023 through Dec-2023)
  === SHEET: FY 2024 ===   (months Jan-2024 through Dec-2024)

Each sheet has this EXACT column structure:
  Column 1: GL Code
  Column 2: Account Name
  Columns 3-14: Jan | Feb | Mar | Apr | May | Jun | Jul | Aug | Sep | Oct | Nov | Dec
  Column 15: STRIPPED (annual total column has been removed before sending)

CRITICAL — DO NOT CONFUSE SHEETS WITH YEARS:
When you see "=== SHEET: FY 2022 ===" followed by "=== SHEET: FY 2023 ===", these are sequential years. Treat the data as a continuous 36-month dataset. FY 2022 = months 1-12, FY 2023 = months 13-24, FY 2024 = months 25-36. The most recent 12 months (FY 2024 = months 25-36) form the TTM period.

DETECTING QB EXPORT FORMAT (within each sheet):
- Company name appears in row 1
- "Profit and Loss" or "Balance Sheet" appears in row 2
- Date range appears in row 3
- Month headers (column names) appear in row 4 or 5
...
DETECTING STANDALONE REPORT FORMAT (within each sheet):
- Month labels may appear in rows 1-3
- Account names may not be indented
- GL codes may or may not be present in a separate column
- Merged cells appear as blank cells following the merged cell value in CSV

For QB Export format:
- IGNORE all section header rows (rows with no dollar values)
- IGNORE all "Total X" or "Net Income" summary rows — you will calculate your own totals
- USE ONLY the leaf-level (most indented) account rows with actual dollar values

For Standalone format:
- Identify the month column positions from the header row
- Map each row to the correct time period

HANDLING MISSING MONTHS:
If a month column is entirely blank or zero across ALL revenue and expense rows within a sheet, flag it as a probable missing data month in the Data Quality Report — do not treat it as a real zero-activity month without flagging.
...
```
- WS2-1 output: structured 36-month P&L model with GL mapping (passed automatically by orchestrator)
- File 5 — Seller Add-Back Disclosure: items 2.1–2.5 and 3.2 combined in one Excel file
    ├── Item 2.1: Shareholder list + all remuneration (36 months, with GL account and code)
    ├── Item 2.2: Personal expenses charged to business (36 months, with GL cross-references)
    ├── Item 2.3: One-off non-recurring expenses $5K+ (with GL cross-references)
    ├── Item 2.4: Material TIs $5K+ (with GL cross-references)
    ├── Item 2.5: Fair Market Rent info (if related-party property ownership)
    └── Item 3.2: Rent reconciliation notes (any rent changes not in the lease)
...
**File routing note for the engineer:** File 5 (Add-Back Disclosure) is uploaded by the seller at the same time as files 1–4. The portal holds it in storage but does NOT pass it to the WS2-1 API call. It is only passed to the WS2-2 API call, after Craig has cleared the WS2-1 HITL gate. This is intentional — WS2-1 does not need the add-back information, and sending it would unnecessarily inflate the token count and risk the agent pre-empting WS2-2's work.

**Format of File 5:** The seller add-back disclosure is a single-sheet Excel file (or multi-section within one sheet). Convert it using `convertSingleSheetExcelToText()` — the same function used for the Accountant Statements and AR Aging files. It will arrive at the WS2-2 agent as a labeled CSV text block.
